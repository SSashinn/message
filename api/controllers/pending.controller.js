...
exports.rejectRequest = [
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (index !== -1) {
      ...
      res.status(404).json({ status: 404, message: "User not in friend request" });
    }
...
exports.acceptRequest = [
  ...
  asyncHandler(async (req, res, next) => {
    ...
    // If the request is found, remove it from the array
    if (index !== -1) {
      const newFriend = user.friend_request.splice(index, 1);
      await user.updateOne({ $push: { friends: newFriend[0]._id } });
    }
